...
describe('TestScheduler', function () {
    ...
    describe('cold()', function () {
        it('should create a cold observable', function () {
            var expected = ['A', 'B'];
            var scheduler = new TestScheduler(null);
            var source = scheduler.cold('--a---b--|', { a: 'A', b: 'B' });
    ...
    describe('hot()', function () {
        it('should create a cold observable', function () {
            var expected = ['A', 'B'];
            var scheduler = new TestScheduler(null);
            var source = scheduler.hot('--a---b--|', { a: 'A', b: 'B' });
    ...
    describe('TestScheduler.run()', function () {
        it('should provide an instance of TestScheduler', function () {
            var called = false;
            TestScheduler.run(function (scheduler) {
                called = true;
                chai_1.expect(scheduler).to.be.an.instanceOf(TestScheduler);
            });
            chai_1.expect(called).to.be.true;
        });
        it('should flush the provided test scheduler after it executes the callback', function () {
            var results = [];
            TestScheduler.run(function (scheduler) {
                var s = scheduler.hot('--^--a--b--c--|');
                s.subscribe(function (x) {
                    results.push(x);
                });
                chai_1.expect(results).to.deep.equal([]);
            });
            chai_1.expect(results).to.deep.equal(['a', 'b', 'c']);
        });
        it('should use TestScheduler.assertDeepEqual to compare observable notifications', function () {
            var called = 0;
            TestScheduler.assertDeepEqual = function (a, b) {
                called++;
                chai_1.expect(a).to.deep.equal(b);
            };
            TestScheduler.run(function (scheduler) {
                chai_1.expect(scheduler.assertDeepEqual).to.equal(TestScheduler.assertDeepEqual);
                var s = scheduler.cold('--a--b--c--|');
                scheduler.expectObservable(s).toBe('--a--b--c--|');
            });
            chai_1.expect(called).to.equal(1);
        });
        it('should patch schedulers provided in TestScheduler.schedulersToPatch', function () {
            var scheduleFn = function (action, delay, state) {
            ...
            TestScheduler.schedulersToPatch = [
                fakeScheduler
            ];
            TestScheduler.run(function (scheduler) {
                chai_1.expect(fakeScheduler.now).not.to.equal(nowFn);
                chai_1.expect(fakeScheduler.schedule).not.to.equal(scheduleFn);
            });
            chai_1.expect(fakeScheduler.now).to.equal(nowFn);
            chai_1.expect(fakeScheduler.schedule).to.equal(scheduleFn);
        });
        it('should run patched Rx library schedulers in the proper order', function () {
            var queue = Rx.Scheduler.queue;
            var asap = Rx.Scheduler.asap;
            var async = Rx.Scheduler.async;
            ...
            TestScheduler.run(function (scheduler) {
                scheduler.schedule(function () { return results.push('testScheduler'); });
                animationFrame.schedule(function () { return results.push('animationFrame'); });
                async.schedule(function () { return results.push('async'); });
                asap.schedule(function () { return results.push('asap'); });
                queue.schedule(function () { return results.push('queue'); });
                chai_1.expect(results).to.deep.equal([]);
            });
            chai_1.expect(results).to.deep.equal(['testScheduler', 'queue', 'asap', 'async', 'animationFrame']);
        });
    });
});
